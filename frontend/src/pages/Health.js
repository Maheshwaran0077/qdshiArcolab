...
const Health = ({ shift }) => {
  ...
  // Only the HEALTH Supervisor can create/edit new entries
  ...
  const [formData, setFormData] = useState({ status: '', keypoints: '', attendance: '' });
  ...
  const handleSave = async () => {
    if (!canUpdate) return; // Protection
    ...
    try {
      ...
      const updatedMonthArray = allMonthsData[currentMonthName].map(item =>
        item.date === selectedDay.date ? { ...item, ...formData } : item
      );
  ...
  const isFormInvalid = !formData.status || 
    (formData.status !== 'holiday' && (!formData.attendance.toString().trim() || !formData.keypoints.trim()));
        ...
        {allMonthsData[currentMonthName].map((item) => {
          ...
          return (
            <div 
              ...
              className={`group relative cursor-pointer rounded-[2rem] h-36 p-5 transition-all duration-300 border-2 hover:scale-[1.03] flex flex-col justify-between ${colorClass} ${isLocked ? 'opacity-80' : ''}`}
            >
              ...
              <div className="overflow-hidden">
                {item.status ? (
                  <div className="space-y-0.5">
                    ...
                    {item.status !== 'holiday' && <p className="text-[10px] font-bold opacity-80 italic">Score: {item.attendance}%</p>}
                  </div>
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md flex items-center justify-center p-4 z-[100] animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] shadow-[0_40px_100px_rgba(0,0,0,0.25)] w-full max-w-md overflow-hidden transform animate-in zoom-in-95 duration-300">
            ...
            <div className="p-8 space-y-6">
              ...
              {formData.status && formData.status !== 'holiday' && (
                <div className="space-y-4 animate-in slide-in-from-bottom-4">
                  <div className="relative">
                    ...
                    />
                    <label className="absolute top-3 left-4 text-[9px] font-black uppercase text-slate-400 tracking-[0.2em]">Attendance %</label>
                  </div>
                  <div className="relative">
                    <textarea 
