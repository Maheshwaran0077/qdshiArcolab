...
const checkAndSendAlerts = async () => {
  try {
    ...
    for (const lock of locks) {
      ...
      if (!hod?.gmail) {
        console.log(`⚠️  No HOD gmail found for dept ${dept} — skipping alert`);
        continue;
      }
      ...
      try {
        ...
        console.log(`📧 Shift alert sent → HOD: ${hod.name} | Dept: ${dept} | Shift: ${shift} | Missed: ${missedModules.join(',')}`);
      } catch (emailErr) {
